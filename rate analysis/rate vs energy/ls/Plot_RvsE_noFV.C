...
void rvse(){
  ...
  fstream file_u238, file_th232, file_k40, file_pb210, file_error;
  file_u238.open("u238_r_vs_e.txt", ios::in);
  file_th232.open("th232_r_vs_e.txt", ios::in);
  file_k40.open("k40_r_vs_e.txt", ios::in);
  file_pb210.open("pb_r_vs_e.txt", ios::in);
  file_error.open("error.txt", ios::in);

  int counter=0;

  while (1) {
    float e_u238, e_th232, e_k40, e_pb210, r_u238, r_th232, r_k40, r_pb210, error;

    file_u238 >> e_u238 >> r_u238;
//    cout << "U238 " << counter << ": " << e_u238 << " " << r_u238 << endl;
    gr_u238->SetPoint(counter,e_u238, r_u238);

    file_th232 >> e_th232 >> r_th2
//  cout << "Th232 " << counter << ": " << e_th232 << " " << r_th232 << endl;
  ...
  //  cout << "Pb210 " << counter << ": " << e_pb210 << " " << r_pb210 << endl;
    ...
    file_error >> error;
    gr_sum->SetPointError(counter,0,100*error);
...
//  c1->SetLogy();
  ...
  mg->GetHistogram()->SetTitle("Rate vs Energy Threshold in Liquid Scintillator without vertex selection");

  c1->Print("R_vs_E_LS_noFV_1000xerror.pdf");
}
