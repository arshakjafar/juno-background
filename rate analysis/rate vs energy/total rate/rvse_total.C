...
void rvse(){
  ...
  file_ls.open("total_rvse_ls_fv.txt", ios::in);
  file_av.open("total_rvse_av_fv.txt", ios::in);
  ...
  mg->GetHistogram()->SetTitle("Rate vs Energy Threshold with FV cut 17.2m");

  c1->Print("R_vs_E_Total.pdf");
